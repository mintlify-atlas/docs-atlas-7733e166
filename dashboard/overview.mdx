---
title: "Traffic Reducer dashboard: a guided tour"
...
- **Prioridad activa** — the direction name (NORTE, SUR, ESTE, or OESTE) with the highest vehicle count, or `EMERGENCIA` if an emergency vehicle is detected.
- **Peatones detectados** — total pedestrian count from the last `/stats` response.
- **Vehículo de emergencia** — mirrors the emergency field; displays `SÍ` or `NO`.

All three cards are updated on every `/stats` poll at a 1 500 ms interval. The stats interval starts automatically after a source is selected and runs independently of the live-mode toggle.
</Accordion>
